...
**Module ID:** `events`
**Version:** 1.0.0
**Status:** In Review — Prompt 2 Agent E, 2026-04-22
...
### 1.1 Purpose (Generic Event Management)

The Event Management module provides comprehensive lifecycle management for organizational events within the Nexora platform. The **core capability set is generic** — it serves corporate workshops, product launches, annual meetings, conferences, customer summits, training sessions, and any other organizational gathering. It handles event creation, registration, attendance tracking, ticketing, venue and speaker management, sponsor tracking, annual calendar administration, and event-based analytics.

On top of this generic core, the module ships an **opt-in NGO seasonal capability** (see §1.4) for tenants on the NGO Edition who need Ramadan Iftar, Qurban distribution, and similar seasonal/religious templates out of the box. Tenants that do not enable the NGO capability see only the generic feature surface.

> **Future direction:** ADR-0016 anticipates that the generic portion of this spec may be extracted into a **Tier-2 generic Events module** once an enterprise customer requires events without the NGO framing. Until then, this module lives in Tier 3a and the generic features are available to NGO-Edition tenants only.

### 1.2 Scope (Generic — available to all tenants with this module installed)

| Capability | Description |
|---|---|
| Event Creation & Management | Full CRUD for events (corporate workshops, product launches, annual meetings, conferences, training) |
...
| Speaker & Venue Management | Speaker profiles, session assignment, venue availability, capacity |
| Sponsor Tracking | Historical sponsor-event association with contribution amounts and recurrence analysis |
| Event Templates | Reusable blueprints for recurring event types (e.g., "Quarterly All-Hands", "Annual User Conference") |
| Calendar Management | Organizational calendar with working days, holidays, and event scheduling |
...
### 1.3 Key Domain Concepts (generic)

- **Event**: A scheduled occurrence with a defined purpose, time range, venue, and capacity.
- **EventCategory**: Classification taxonomy for events (corporate, training, social, partner, community, …).
- **EventSession**: A discrete time block within a multi-session event (e.g., Day 1 Keynote, Day 2 Workshops).
...
- **EventTemplate**: A reusable blueprint for recurring event types (e.g., Quarterly All-Hands, Annual User Conference).
- **CalendarEntry**: An item on the organizational calendar (event-linked or standalone).

### 1.4 Seasonal / NGO-specific Templates (Opt-in)

> **Opt-in capability.** The following features are only surfaced when the NGO Edition is active **and** the tenant enables the `events.seasonal.enabled` feature flag. They are **not** part of the generic event management surface and MUST NOT be referenced by Tier-1 or Tier-2 consumers.

When enabled, the module ships a pre-populated library of **seasonal templates** tailored to Islamic-calendar and NGO fundraising use cases:

| Seasonal Template | Purpose |
|---|---|
| Ramadan Iftar | Nightly iftar meals during Ramadan with per-night session scheduling |
| Sahur | Pre-dawn meal events during Ramadan |
| Qurban Distribution | Eid al-Adha meat distribution coordination |
| Fundraiser Dinner / Gala | Ticketed fundraising dinner with donation integration |
| Bazaar | Multi-day charity bazaar with vendor slots |

These templates reuse the generic `EventTemplate`, `EventSession`, and `Registration` entities — the "seasonal" nature is captured purely via the `season_tag` column on `events_event_template` and the optional `SeasonTag` enum (see §10.2). Disabling the capability hides the seasonal template library and associated reports but does not remove any core functionality.
...
| `fundraising` | Optional | Linking event sponsorships to donation records, pledge tracking |
...
erDiagram
    ...
    events_ticket_type {
        ...
        varchar(200) name
        text description
        Money ticket_price "Money(decimal Amount, string CurrencyCode) — SharedKernel value object"
        int total_quantity
        int sold_count
        int reserved_count
    ...
    events_event_sponsor {
        ...
        Money contribution_amount "Money(decimal Amount, string CurrencyCode) — SharedKernel value object"
...
Every root entity includes a `tenant_id` column. Tenant isolation is enforced via EF Core global query filters on `TenantId` and `IsDeleted` — there are no PostgreSQL RLS policies. The `TenantContext` is injected into `EventDbContext` and applied automatically to all queries:

```csharp
// Tenant isolation: EF Core global query filter on TenantId applied in EventDbContext
modelBuilder.Entity<NexoraEvent>().HasQueryFilter(e => e.TenantId == _tenantContext.TenantId && !e.IsDeleted);
```
...
sequenceDiagram
    ...
    participant DonMod as Fundraising Module (optional)
    ...
    alt Cache Hit
        ...
        opt Fundraising module available
...
| `EventCancelledIntegrationEvent` | `{ EventId, TenantId, Title, CancellationReason, AffectedRegistrationCount }` | `notifications`, `crm`, `fundraising` |
...
| `EventCompletedIntegrationEvent` | `{ EventId, TenantId, TotalAttendees, TotalRegistrations, TotalRevenue }` | `crm`, `fundraising`, `notifications` |
| `SponsorshipRecordedIntegrationEvent` | `{ EventId, SponsorId, ContactId, TenantId, Amount, Currency, Level }` | `fundraising`, `crm`, `contacts` |
...
| `DonationReceivedIntegrationEvent` | `fundraising` | `DonationReceivedHandler` | Correlate donation with event sponsorship if metadata includes eventId |
...
- **Data isolation**: EF Core global query filters on `TenantId` and `IsDeleted` enforce tenant boundary at the application level. All queries are automatically scoped to the current tenant via `TenantContext` injected into `EventDbContext`; cross-tenant data leakage is prevented at the query-filter layer.
...
_Status: In Review — Prompt 2 Agent E, 2026-04-22_
