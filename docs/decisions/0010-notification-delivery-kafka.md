...
- Idempotency key format: `{tenantId}:{notificationId}:{recipientId}:{channel}` — persisted in `notification_delivery_attempts` table.
- Consumer checks the key before sending; duplicate Kafka delivery results in a no-op log entry, not a second send.
- Retry policy: 3 exponential retries (1min, 5min, 15min) within the consumer on transient failures (SMTP timeout, rate limit).
- Dead-letter queue: after retry exhaustion, the message is published to `nexora.notifications.dlq` topic with original payload + failure metadata. Ops alert fires on DLQ depth > 100.
- Poison-pill detection: malformed messages failing schema validation go straight to DLQ (no retry).
- DLQ replay: manual via `notifications:replay-dlq` Hangfire ad-hoc job with operator approval.
