...
### ICompensationHandler abstraction

To avoid scattered try/catch compensation code across handlers, Tier-2B flows implement `ICompensationHandler<TCommand>` from SharedKernel:

```csharp
public interface ICompensationHandler<in TCommand>
{
    Task CompensateAsync(TCommand command, Exception failure, CancellationToken ct);
}
```

- Implementations are registered in DI alongside the command handler.
- `CqrsMiddleware` invokes the compensation handler on external-call failure (Tier-2B only).
- Compensation handlers MUST be idempotent — invoked at least once, possibly retried by the outbox.
- Compensation handlers publish a `{Module}.{Action}.CompensatedEvent` for observability.

**Keycloak orphan reconciliation:** The recurring job `identity:keycloak-reconciliation` runs daily at 02:00 UTC (per tenant), comparing Keycloak users to local `users` table. Orphans (exist in Keycloak but not locally, or vice versa) are logged and optionally cleaned based on `identity.reconciliation.auto_cleanup` tenant config (default false). Job target: Phase-2 Milestone B.
