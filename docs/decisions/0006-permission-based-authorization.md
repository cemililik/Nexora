...
- Cache backend: `ICacheService` (per CLAUDE.md Infrastructure Standards — never `IDistributedCache` or `DaprClient` directly).
- Cache key format: `identity:user-permissions:{userId}` (tenant prefix auto-prepended by `DaprCacheService`).
- TTL: 5 minutes L2 (Redis via Dapr), 2 minutes L1 (in-memory).
- Invalidation API: `IPermissionCacheInvalidator.InvalidateUser(userId)` and `.InvalidateRole(roleId)` (invalidates all users with that role). Both are idempotent.
- Cross-instance invalidation: delegated to ADR-0013 (`CacheInvalidationEvent` Dapr pub/sub).
- Eviction triggers: role assignment changes, role permission changes, user deactivation.
- Worst-case staleness: 5 minutes if invalidation event is lost (fallback TTL).

## Related
- [ADR-004: Centralized Permission Seeding](./0004-centralized-permission-seeding.md)
