...
### Route-based tenantId architecture test

An architecture test (`tests/Nexora.Architecture.Tests/RouteTenantIdTests.cs`) scans all `[HttpGet/Post/Put/Delete]` endpoints and FAILS the build if a `{tenantId:guid}` route parameter is found outside an explicit allowlist:

- Allowlist (Platform-scope endpoints only): NMP admin APIs under `Nexora.Management.*`, cross-tenant audit views, super-admin GDPR export endpoints.
- All other endpoints MUST resolve tenant from `ITenantContextAccessor` (JWT claim), not from route.
- Violation example: `[HttpGet("/api/v1/{tenantId:guid}/contacts")]` in a tenant-scoped module → build fail.

Implementation target: Phase-2 Milestone A.

**Interim state (pre-Phase-1.5.2):** Platform-scope endpoints are protected by explicit `[Authorize(Policy = "platform.admin")]` attribute; no generic Platform-scope enforcement until `PermissionScope` enum ships.
...
- [ADR-006: Permission-Based Authorization](0006-permission-based-authorization.md)
